...
library SignatureVerifier {
    ...
    // function makeSignatureHash2(address signature, bytes memory message bytes memory request) internal pure returns(bytes32) {
    // return keccak256(abi.encodePacked(hex"1900", signature, message, keccak256(request)));
    ...
    // function verify2(bytes calldata request, bytes calldata response) internal view returns(string memory, address, uint256, uint256, uint256, address) {
    //     //Decode response
    //     (string memory signatureData, bytes memory messageData) = abi.decode(response, (string, bytes));

    //     //Decode message
    //     (address sender, uint256 amount, uint256 nonce, uint256 expire) = abi.decode(messageData, (address, uint256, uint256, uint256));

    //     //Verify signature
    //     address signer = ECDSA.recover(makeSignatureHash2(
    //         signature, messageData, request
    //     ), );

    //     return(signature, sender, amount, nonce, expire, signer);
